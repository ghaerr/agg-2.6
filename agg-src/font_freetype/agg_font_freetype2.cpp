...
//------------------------------------------------------------------------
static inline int dbl_to_plain_fx(double d) { return int(d * 65536.0); }

//------------------------------------------------------------------------
static inline double int26p6_to_dbl(int p) { return double(p) / 64.0; }

//------------------------------------------------------------------------
static inline int dbl_to_int26p6(double p) { return int(p * 64.0 + 0.5); }

//------------------------------------------------------------------------
template <class PathStorage>
bool decompose_ft_outline(const FT_Outline &outline, bool flip_y,
                          const trans_affine &mtx, PathStorage &path) {
  ...
  FT_Vector *point = nullptr;
  FT_Vector *limit = nullptr;
  char *tags = nullptr;

  int n = 0;// index of contour in outline
  int first = 0;// index of first point in contour
  char tag = 0;// current point's state

  first = 0;

  for (n = 0; n < outline.n_contours; n++) {
    int last = 0;// index of last point in contour
      ...
      Do_Conic:
        if (point < limit) {
          ...
          if (tag == FT_CURVE_TAG_ON) {
            ...
            mtx.transform(&x2, &y2);
            path.curve3(
                value_type(dbl_to_int26p6(x1)), value_type(dbl_to_int26p6(y1)),
                value_type(dbl_to_int26p6(x2)), value_type(dbl_to_int26p6(y2)));
            continue;
          }
      ...
      default: // FT_CURVE_TAG_CUBIC
      {
        ...
        if (point <= limit) {
          ...
          path.curve4(
              value_type(dbl_to_int26p6(x1)), value_type(dbl_to_int26p6(y1)),
              value_type(dbl_to_int26p6(x2)), value_type(dbl_to_int26p6(y2)),
              value_type(dbl_to_int26p6(x3)), value_type(dbl_to_int26p6(y3)));
...
void decompose_ft_bitmap_mono(const FT_Bitmap &bitmap, int x, int y,
                              bool flip_y, Scanline &sl,
                              ScanlineStorage &storage) {
  int i = 0;
  const auto *buf = (const int8u *)bitmap.buffer;
...
void decompose_ft_bitmap_gray8(const FT_Bitmap &bitmap, int x, int y,
                               bool flip_y, Rasterizer &ras, Scanline &sl,
                               ScanlineStorage &storage) {
  int i = 0;
  int j = 0;
  const auto *buf = (const int8u *)bitmap.buffer;
...
//------------------------------------------------------------------------
bool font_engine_freetype_base::loaded_face::prepare_glyph(
    unsigned glyph_code, prepared_glyph *prepared) const {
  bool success = false;
  prepared->glyph_index = FT_Get_Char_Index(m_ft_face, glyph_code);
  int error = FT_Load_Glyph(m_ft_face, prepared->glyph_index,
  ...
  if (error == 0) {
    ...
    case glyph_ren_native_mono:
      ...
      if (error == 0) {
        decompose_ft_bitmap_mono(
            m_ft_face->glyph->bitmap, m_ft_face->glyph->bitmap_left,
            m_flip_y ? -m_ft_face->glyph->bitmap_top
                     : m_ft_face->glyph->bitmap_top,
            m_flip_y, m_engine.m_scanline_bin, m_engine.m_scanlines_bin);
    ...
    case glyph_ren_native_gray8:
      ...
      if (error == 0) {
        decompose_ft_bitmap_gray8(
            ...
            m_flip_y ? -m_ft_face->glyph->bitmap_top
                     : m_ft_face->glyph->bitmap_top,
            m_flip_y, m_engine.m_rasterizer, m_engine.m_scanline_aa,
    ...
    case glyph_ren_outline:
      if (error == 0) {
        if (m_engine.m_flag32) {
          ...
          if (decompose_ft_outline(m_ft_face->glyph->outline, m_flip_y,
                                   m_affine, m_engine.m_path32)) {
    ...
    case glyph_ren_agg_mono:
      if (error == 0) {
        ...
        if (m_engine.m_flag32) {
          m_engine.m_path32.remove_all();
          decompose_ft_outline(m_ft_face->glyph->outline, m_flip_y, m_affine,
                               m_engine.m_path32);
        ...
        render_scanlines(m_engine.m_rasterizer, m_engine.m_scanline_bin,
    ...
    case glyph_ren_agg_gray8:
      if (error == 0) {
        ...
        render_scanlines(m_engine.m_rasterizer, m_engine.m_scanline_aa,
                         m_engine.m_scanlines_aa);
        prepared->bounds.x1 = m_engine.m_scanlines_aa.min_x();
        prepared->bounds.y1 = m_engine.m_scanlines_aa.min_y();
        prepared->bounds.x2 = m_engine.m_scanlines_aa.max_x() + 1;
...
//------------------------------------------------------------------------
void font_engine_freetype_base::loaded_face::write_glyph_to(
    prepared_glyph *prepared, int8u *data) const {
  if ((data != nullptr) && (prepared->data_size != 0U)) {
    switch (prepared->data_type) {
    default:
      return;
    case glyph_data_mono:
      m_engine.m_scanlines_bin.serialize(data);
      break;
    case glyph_data_gray8:
      m_engine.m_scanlines_aa.serialize(data);
      break;
...
//------------------------------------------------------------------------
bool font_engine_freetype_base::loaded_face::add_kerning(unsigned first,
                                                         unsigned second,
                                                         double *x, double *y) {
  bool success = false;
  if ((first != 0U) && (second != 0U) && FT_HAS_KERNING(m_ft_face)) {
    FT_Vector delta;
    int error =
        FT_Get_Kerning(m_ft_face, first, second, FT_KERNING_DEFAULT, &delta);
    if (error == 0) {
...
void font_engine_freetype_base::loaded_face::set_face_name() {
  if (strncmp(m_ft_face->style_name, "Regular", 7) == 0) {
    std::size_t len = std::strlen(m_ft_face->family_name) + 1;
    m_face_name = new char[len];
    std::strcpy(m_face_name, m_ft_face->family_name);
  } else {
    std::size_t len = std::strlen(m_ft_face->family_name) + 1 +
                      std::strlen(m_ft_face->style_name) + 1;
    m_face_name = new char[len];
    std::sprintf(m_face_name, "%s %s", m_ft_face->family_name,
                 m_ft_face->style_name);
  }
}
...
font_engine_freetype_base::font_engine_freetype_base(bool flag32,
  void *ftMemory)
  : m_flag32(flag32), m_last_error(ft_init_freeType(&m_library, FT_Memory(ftMemory))), m_library_initialized(false),
    m_curves16(m_path16), m_curves32(m_path32)
...
//------------------------------------------------------------------------
font_engine_freetype_base::loaded_face *
font_engine_freetype_base::load_face(const void *buffer, std::size_t bytes) {
  loaded_face *face = nullptr;
  if (m_library_initialized) {
    FT_Face ft_face = nullptr;
    int error = FT_New_Memory_Face(m_library, (const FT_Byte *)buffer, bytes, 0,
                                   &ft_face);
    if (error == 0) {
...
//------------------------------------------------------------------------
font_engine_freetype_base::loaded_face *
font_engine_freetype_base::load_face_file(const char *file_name) {
  loaded_face *face = nullptr;
  if (m_library_initialized) {
    FT_Face ft_face = nullptr;
    int error = FT_New_Face(m_library, file_name, 0, &ft_face);
    if (error == 0) {
      face = create_loaded_face(ft_face);
    }
  }
...
//------------------------------------------------------------------------
font_engine_freetype_base::loaded_face *
font_engine_freetype_base::create_loaded_face(FT_Face ft_face) {
  auto *face = new loaded_face(*this, ft_face);
