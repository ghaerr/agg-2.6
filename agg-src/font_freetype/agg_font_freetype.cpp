...
font_engine_freetype_base::font_engine_freetype_base(bool flag32,
  unsigned max_faces) : m_flag32(flag32),
                        m_change_stamp(0),
...
int font_engine_freetype_base::find_face(const char *face_name) const
{
  unsigned i = 0;
  for (i = 0; i < m_num_faces; ++i) {
    if (std::strcmp(face_name, m_face_names[i]) == 0) {
      return i;
    }
  }
  return -1;
  ...
  const long font_mem_size)
{
  ...
  if (m_library_initialized) {
    ...
    if (idx >= 0) {
      ...
      if ((font_mem != nullptr) && (font_mem_size != 0)) {
      ...
      if (m_last_error == 0) {
        ...
        m_face_names[m_num_faces] = nullptr;
        m_cur_face = nullptr;
        m_name = nullptr;
      }
    }
...
{
  if ((m_cur_face != nullptr) && (m_name != nullptr)) {
    ...
    if (m_glyph_rendering == glyph_ren_native_gray8 || m_glyph_rendering == glyph_ren_agg_mono || m_glyph_rendering == glyph_ren_agg_gray8) {
    ...
    std::sprintf(m_signature,
      ...
      int(m_flip_y),
      gamma_hash);
    if (m_glyph_rendering == glyph_ren_outline || m_glyph_rendering == glyph_ren_agg_mono || m_glyph_rendering == glyph_ren_agg_gray8) {
      double mtx[6];
      char buf[100];
      m_affine.store_to(mtx);
      std::sprintf(buf, ",%08X%08X%08X%08X%08X%08X", dbl_to_plain_fx(mtx[0]), dbl_to_plain_fx(mtx[1]), dbl_to_plain_fx(mtx[2]), dbl_to_plain_fx(mtx[3]), dbl_to_plain_fx(mtx[4]), dbl_to_plain_fx(mtx[5]));
    ...
    case glyph_ren_native_mono:
      ...
      if (m_last_error == 0) {
        decompose_ft_bitmap_mono(m_cur_face->glyph->bitmap,
          ...
          m_flip_y ? -m_cur_face->glyph->bitmap_top : m_cur_face->glyph->bitmap_top,
          m_flip_y,
          m_scanline_bin,
          m_scanlines_bin);
...
{
  if ((data != nullptr) && (m_data_size != 0U)) {
    switch (m_data_type) {
    default:
      return;
...
//------------------------------------------------------------------------
bool font_engine_freetype_base::add_kerning(unsigned first, unsigned second, double *x, double *y)
{
  if ((m_cur_face != nullptr) && (first != 0U) && (second != 0U) && FT_HAS_KERNING(m_cur_face)) {
    FT_Vector delta;
