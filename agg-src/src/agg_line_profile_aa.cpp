...
void line_profile_aa::set(double center_width, double smoother_width)
{
  ...
  auto subpixel_center_width = unsigned(center_width * subpixel_scale);
  auto subpixel_smoother_width = unsigned(smoother_width * subpixel_scale);
  ...
  for (i = 0; i < subpixel_smoother_width; i++) {
    *ch_smoother++ =
      m_gamma[unsigned((base_val - base_val * (double(i) / subpixel_smoother_width)) * aa_mask)];
  }
