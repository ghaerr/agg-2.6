...
//------------------------------------------------------------------------
void curve3_inc::rewind(unsigned /*unused*/)
{
  if (m_num_steps == 0) {
    m_step = -1;
    return;
...
void curve4_inc::init(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4)
{
  ...
  double len = (std::sqrt(dx1 * dx1 + dy1 * dy1) + std::sqrt(dx2 * dx2 + dy2 * dy2) + std::sqrt(dx3 * dx3 + dy3 * dy3)) * 0.25 * m_scale;
...
//------------------------------------------------------------------------
void curve4_inc::rewind(unsigned /*unused*/)
{
  if (m_num_steps == 0) {
    m_step = -1;
    return;
